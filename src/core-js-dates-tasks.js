...
function isDateInPeriod(date, period) {
  const myDate = Date.parse(new Date(date));
  const startDate = Date.parse(new Date(period.start));
  const endDate = Date.parse(new Date(period.end));
...
function formatDate(date) {
  ...
  const hours = myDate.getUTCHours() % 12 || 12;
  const minutes = `0${myDate.getUTCMinutes()}`.slice(-2);
  const seconds = myDate.getUTCSeconds();
  const timeOfDay = myDate.getUTCHours() >= 12 ? 'PM' : 'AM';

  return `${month}/${day}/${year}, ${hours}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')} ${timeOfDay}`;
}
...
function getNextFridayThe13th(date) {
  const yearFIND = date.getUTCFullYear();
  const monthFIND = date.getUTCMonth();
  ...
  function find(year, month) {
    ...
    if (month >= 12) {
      return find(year + 1, 1);
    }
    return find(year, month + 1);
  }

  return find(yearFIND, monthFIND);
}
